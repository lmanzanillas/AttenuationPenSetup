...
void macro_read_data(TString stringDataName = "output/Bi207-17_04_2020-14-10-21", int nCores = 1){ 
	...
	TH1D* h_edep_PEN_all = new TH1D("h_edep_PEN_all","h_edep_PEN_all;Edep [keV]; entries / 2.5 keV ",600,10.,2010.);
        TH1D* h_total_charge = new TH1D("h_total_charge","h_total_charge; charge [number of photons];entries / photon ",200,5.,805.);
	...
	calls.Draw("(EdepPEN1+EdepPEN2+EdepPEN3)>>h_edep_PEN_all","(NPMT1+NPMT2)>1.&&NPMT1>1&&NPMT2>1&&EdepTrigger>20&&EdepTrigger<100");
        ...
        calls.Draw("(NPMT1+NPMT2)>>h_total_charge","(NPMT1+NPMT2)>1.&&NPMT1>1&&NPMT2>1&&EdepTrigger>20&&EdepTrigger<100");
